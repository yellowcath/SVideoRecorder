...
public abstract class MediaEncoder implements Runnable {
    ...
    public MediaEncoder(final MediaMuxer muxer, final MediaEncoderListener listener, CountDownLatch countDownLatch) {
        if (listener == null) {
...
//********************************************************************************
     ...
     */
    protected void release(boolean releaseMuxer) {
        if (DEBUG) {
            Log.d(TAG, "release:");
        }
        try {
        ...
        if (mMuxerStarted) {
            RL.i("releaseMuxer:"+releaseMuxer);
            if (!releaseMuxer) {
                SMediaCodecRecorder.sStopSemaphore.release(1);
            } else if (mMediaMuxer != null) {
                try {
                    SMediaCodecRecorder.sStopSemaphore.release(1);
//                    RL.it("MediaEncoder", "release1 acquire,availablePermits:" + SMediaCodecRecorder.sStopSemaphore.availablePermits());
                    SMediaCodecRecorder.sStopSemaphore.acquire(2);
 //                   RL.i("MediaEncoder,acquire,ok:");
                    if (!SMediaCodecRecorder.sMuxerStopped) {
                        SMediaCodecRecorder.sMuxerStopped = true;
                        RL.i("MediaEncoder,stop");
                        mMediaMuxer.stop();
                    }
                } catch (final Exception e) {
                    Log.e(TAG, "failed stopping muxer", e);
                } finally {
//                    L.it("VideoMediaEncoderThread", "+ mMediaMuxer.release()");
                    mMediaMuxer.release();
//                    L.it("VideoMediaEncoderThread", "- mMediaMuxer.release()");
                    SMediaCodecRecorder.sStopSemaphore.release(2);
//                    L.it("VideoMediaEncoderThread", "MediaEncoder,-release");
                }
            }
        } else{
            if(!releaseMuxer){
...
//        	throw new NullPointerException("muxer is unexpectedly null");
        ...
        while (mIsCapturing) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                ...
                mNewFormat = mMediaCodec.getOutputFormat(); // API >= 16
                ...
                if (mBufferInfo.size != 0) {
                    ...
                    count = 0;
                    if (!mMuxerStarted) {
                        mTrackIndex = mMediaMuxer.addTrack(mNewFormat);
                        mMuxerStarted = true;
                        SMediaCodecRecorder.sStartSemaphore.release(1);
                        try {
