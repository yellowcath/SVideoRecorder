...
public class MediaAudioEncoder extends MediaEncoder {
    ...
    public MediaAudioEncoder(final MediaMuxer muxer, final MediaEncoderListener listener, CountDownLatch countDownLatch) {
        super(muxer, listener, countDownLatch);
    }
                                ...
                                for (; mIsCapturing && !mRequestStop && !mIsEOS; ) {
                                    ...
                                    if (readBytes > 0) {
                                        ...
                                        if (mRecordSilentAudio) {
                                            ...
                                            emptpBuf.flip();
                                            encode(emptpBuf, readBytes, pts);
                                        } else {
                                            encode(buf, readBytes, pts);
                                        }
                                        frameAvailableSoon();
                                    } else {
                                        SMediaCodecRecorder.sStartSemaphore.release();
